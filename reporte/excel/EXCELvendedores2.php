...
$objPHPExcel->getActiveSheet()->getStyle($az[$iniciadinamica]."5:".$az[($i-1)]."5")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('FFEBF1DE');
...
foreach($rpt as $r){	
	...
	if( $countrpt3==1 ){
		...
		$objPHPExcel->getActiveSheet()->setCellValue($az[$iniciadinamica].$valorinicial, '=SUM('.$az[($iniciadinamica+1)].$valorinicial.':'.$az[($iniciadinamica+count($rpt3)*1+1-1)].$valorinicial.")");
		
		$objPHPExcel->getActiveSheet()->setCellValue($az[($iniciadinamica+count($rpt3)*1+1)].$valorinicial, "=SUM(".$az[($iniciadinamica+count($rpt3)*1+1+1)].$valorinicial.":".$az[($iniciadinamica+count($rpt3)*2+2-1)].$valorinicial.")");
		$objPHPExcel->getActiveSheet()->setCellValue($az[($iniciadinamica+count($rpt3)*2+2)].$valorinicial, '=SUM('.$az[($iniciadinamica+count($rpt3)*2+2+1)].$valorinicial.":".$az[($iniciadinamica+count($rpt3)*3+3-1)].$valorinicial.")");
...
$objPHPExcel->getActiveSheet()->getStyle($az[($iniciadinamica+count($rpt3)*1+1)]."6:".$az[($iniciadinamica+count($rpt3)*1+1)].$valorinicial)->applyFromArray($styleBold);
///negrita a los numeros
//$objPHPExcel->getActiveSheet()->getStyle("C6:".$az[(2+count($rpt3)*2+2-1)].$valorinicial)->applyFromArray($styleBold);
$objPHPExcel->getActiveSheet()->getStyle($az[($iniciadinamica+count($rpt3)*2+2)]."6:".$az[($iniciadinamica+count($rpt3)*2+2)].$valorinicial)->applyFromArray($styleBold);
//$objPHPExcel->getActiveSheet()->getStyle($az[($iniciadinamica+count($rpt3)*3+3)]."6:".$az[($iniciadinamica+count($rpt3)*3+3)].$valorinicial)->applyFromArray($styleBold);
