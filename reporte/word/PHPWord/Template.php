...
class PHPWord_Template {
    ...
    public function cloneRowPos($search,$position, $numberOfClones) {
        if(substr($search, 0, 2) !== '${' && substr($search, -1) !== '}') {
            $search = '${'.$search.'}';
        }
        ...
        $tagPosP      = strpos($this->_documentXML, $position);
        $rowStartPosP = strrpos($this->_documentXML, "<w:tr>", ((strlen($this->_documentXML) - $tagPosP) * -1));
        $rowEndPosP   = strpos($this->_documentXML, "</w:tr>", $tagPosP) + 7;

        $result = substr($this->_documentXML, 0, $rowStartPosP);
        $xmlRow = substr($this->_documentXML, $rowStartPos, ($rowEndPos - $rowStartPos));
        for ($i = 1; $i <= $numberOfClones; $i++) {
            $result .= preg_replace('/\$\{(.*?)\}/','\${\\1#'.$i.'}', $xmlRow);
        }
        $result .= substr($this->_documentXML, $rowEndPosP);
